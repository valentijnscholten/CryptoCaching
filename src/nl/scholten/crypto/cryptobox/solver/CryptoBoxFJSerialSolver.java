...
public class CryptoBoxFJSerialSolver extends RecursiveTask<CryptoBoxFJSolver> {
...
//	private static final boolean SCORE_JAVA_REGEX = true;
	...
	protected Set<CryptoBoxFJSolver> maxScorersSet;
	private int serialStepsLeft;
	private Double bruteTries;
	
	protected long found;

	public CryptoBoxFJSerialSolver(String input, int size, int stepsLeft, List<String> hits, List<String> beginnings, List<String> endings, 
	...
	protected void init(String input, int size, int stepsLeft, List<String> hits, List<String> beginnings, List<String> endings, 
		...
		if (headStarts == null || headStarts.isEmpty()) {
			...
			this.headStarts.add(new ArrayList<CryptoBoxFJSolver.OperationInstance>());
		}
		this.size = size;
		this.opsLog = new ArrayList<CryptoBoxFJSolver.OperationInstance>();
		this.ois = getOisAll();
		this.stepsLeft = stepsLeft;
		this.bruteTries = Math.pow(40d, stepsLeft);

		this.maxScore = 0;
		this.maxScorersSet = Collections.synchronizedSet(new HashSet<CryptoBoxFJSolver>());
	...
	public CryptoBoxFJSerialSolver(CryptoBoxFJSolver m) {
		this.init(m);
	}

	protected void init(CryptoBoxFJSolver m) {
		...
		this.maxScorersSet = new HashSet<CryptoBoxFJSolver>(m.maxScorersSet);
	}
	...
	public static String toStringSideBySide(CryptoBoxFJSolver... matrices) {
		...
		for (int row = 0; row < matrices[0].size; row++) {
			for (CryptoBoxFJSolver m : matrices) {
				if (result.length() > 0
	...
	private void logResult(CryptoBoxFJSolver winner) {
		...
		System.out.println("Total tries serial: " + winner.tries + " maxScore: "
				...
				+ "maxScorers: "
				+ ((CryptoBoxFJSolver)winner.maxScorersSet.toArray()[0]).opsLog);
		
		System.out.println("maxScorers: " + winner.maxScorersSet.size() + ": "+ winner.maxScorersSet);
		
		for (CryptoBoxFJSolver maxScorer: winner.maxScorersSet){
	...
	public void solveSerially() {
		...
		preStart();
		CryptoBoxFJSolver winner = solveSeriallyInternal();
...
//		Matrix winner = maxScorers.get(0);
	...
	public void solveFJ() {
		...
		CryptoBoxFJSolver winner = mainPool.invoke(m);

		logResult(winner);
	}

	private void logProgress(CryptoBoxFJSolver m, boolean force) {
		long now = System.currentTimeMillis();
		...
		if (force || effectiveTries % 10000000l == 0) {
			System.out.println("Current tries: " + effectiveTries + " ("
					...
					+ m.opsLog + ((CryptoBoxFJSolver)m.maxScorersSet.toArray()[0]).opsLog + m.maxScorersSet);
			System.out.println("current data: " + ((CryptoBoxFJSolver)m.maxScorersSet.toArray()[0]).data + " " + m.toString());
		}
	}
	...
	private void doScoring() {
		...
		if (GATHER_DUPLICATES) {
			if (score > 0) {
				CryptoBoxFJSolver m2 = new CryptoBoxFJSolver(this);
	...
	private void processScore(int score) {
		...
		if (score >= maxglobal) {
			...
			if (maxScoreGlobal.compareAndSet(maxglobal, score)) {
				final CryptoBoxFJSolver winner;
				if (score > maxScore) {
					maxScorersSet.clear();

					maxScore = score;
					found = System.currentTimeMillis();
					winner = new CryptoBoxFJSolver(this);
					...
					maxScorersSet.add(new CryptoBoxFJSolver(this));
					//filter set
					
					Set<CryptoBoxFJSolver> newMaxScorers = new HashSet<CryptoBoxFJSolver>();
					int newMaxScore = -1;
					for (CryptoBoxFJSolver m: maxScorersSet) {
	...
	private CryptoBoxFJSolver solveSeriallyInternal() {
		//no headstart -> use empty headstart
		if (headStarts == null || headStarts.isEmpty()) return solvedSeriallyInternalHeadStart(new ArrayList<CryptoBoxFJSolver.OperationInstance>());
		
		List<CryptoBoxFJSolver> partialResults = new ArrayList<CryptoBoxFJSolver>();
		for(List<OperationInstance> headStart: headStarts) {
			
			CryptoBoxFJSolver m = new CryptoBoxFJSolver(this);
			m.headStarts = null;
			m.start = start;
	...
	private CryptoBoxFJSolver joinResults(List<CryptoBoxFJSolver> partialResults) {
		int joinedMaxScore = -1;
		long sumTries = 0;
		CryptoBoxFJSolver winner = null;
		Set<CryptoBoxFJSolver> joinedMaxScorers = new HashSet<CryptoBoxFJSolver>();

		for(CryptoBoxFJSolver m: partialResults){
			int score = m.score();
			sumTries += m.tries;
		...
		winner.maxScorersSet = new HashSet<CryptoBoxFJSolver>(joinedMaxScorers);
		winner.start = start;
	...
	private CryptoBoxFJSolver solvedSeriallyInternalHeadStart(List<OperationInstance> headStart) {
		apply(headStart);
		if (stepsLeft == 0) {
			...
			CryptoBoxFJSolver winner = ((CryptoBoxFJSolver)maxScorersSet.toArray()[0]);
			return winner;
		}
		return doSolveSeriallyInternalNextStep();
	}
	
	private CryptoBoxFJSolver doSolveSeriallyInternalNextStep() {
			...
			if (prevOIA != prevOIB || (oi != prevOIA || oi != prevOIB)) {
				...
				if (	//make sure all segments of the same type (row/col) are in ascending order. Order doesn't matter, so only calculate for those with ascending order)
						...
						if (!USE_FUZZY || !USE_FUZZY_RANDOM_SKIP || Math.random() * 100 > USE_FUZZY_RANDOM_SKIP_PERCENTAGE) {
							apply(oi);
							CryptoBoxFJSolver result = solveSeriallyInternal();
							unapply(oi);
						}
				}
			 }
	...
	public CryptoBoxFJSolver compute() {
		int l = ois.size();
		int s = stepsLeft;
		...
		if (ois.size() == 0) {
			...
			CryptoBoxFJSolver m1 = new CryptoBoxFJSolver(this);
			...
			if (stepsLeft > serialStepsLeft) {
				...
				List<RecursiveTask<CryptoBoxFJSolver>> tasks = new ArrayList<RecursiveTask<CryptoBoxFJSolver>>();
				...
				List<RecursiveTask<CryptoBoxFJSolver>> results = (List<RecursiveTask<CryptoBoxFJSolver>>) invokeAll(tasks);

				List<CryptoBoxFJSolver> partialResults = new ArrayList<CryptoBoxFJSolver>();
				for (RecursiveTask<CryptoBoxFJSolver> task : results) {
					CryptoBoxFJSolver m = task.getRawResult();
				...
				CryptoBoxFJSolver partWinner = solveSeriallyInternal();
				return partWinner;
			}
		}
	}
